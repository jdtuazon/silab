...
import { ZoomIn, ZoomOut, FileText, Target, Shield, Building2, Users, Info } from "lucide-react"
...
export function DocumentViewer({ 
  ...
  const getSectionTypeIcon = (type: string) => {
    switch (type) {
      case "feature": return <Target className="h-3 w-3" />
      case "compliance": return <Shield className="h-3 w-3" />
      case "business": return <Building2 className="h-3 w-3" />
      case "data_privacy": return <Users className="h-3 w-3" />
      case "architecture": return <FileText className="h-3 w-3" />
      default: return <Info className="h-3 w-3" />
    }
  }

  const getSectionStyles = (section: SectionAnalysis, isSelected: boolean) => {
    const baseStyles = "cursor-pointer transition-all duration-200 border-l-4 hover:bg-opacity-30"
  ...
  const findSectionForLine = (lineNumber: number): SectionAnalysis | null => {
    ...
    return semanticAnalysisData.section_analyses.find(section => 
  ...
  return (
    <Card className="h-full max-h-[calc(100vh-180px)] flex flex-col">
      ...
      <CardContent className="flex-1 min-h-0 overflow-hidden p-0">
        {content ? (
          <div className="h-full overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
            <div className="p-6">
              <div className="font-mono text-sm lg:text-sm text-xs leading-relaxed space-y-0">
                {lines.map((line, index) => {
                  const lineNumber = index + 1
                  const section = findSectionForLine(lineNumber)
                  const isSelected = selectedSection && section && 
                    section.sectionTitle === selectedSection.sectionTitle
                  const isFirstLineOfSection = section && section.startLine === lineNumber
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        "flex group",
                        section && getSectionStyles(section, isSelected || false)
                      )}
                      onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
                      ...
                      <div className="flex-1 min-h-[1.5rem] flex items-center">
                        {isFirstLineOfSection && section && (
                          <div className="inline-flex items-center gap-1 mr-2 px-1.5 py-0.5 rounded text-xs font-medium bg-white bg-opacity-80 border">
                            ...
                            <span className="text-gray-700">{section.sectionTitle}</span>
      ...
      {content && (semanticAnalysisData?.section_analyses?.length || violations.length > 0) && (
        <div className="flex-shrink-0 border-t border-border p-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 lg:gap-0 text-sm">
            {semanticAnalysisData?.section_analyses && semanticAnalysisData.section_analyses.length > 0 ? (
              // Semantic analysis footer
              <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4">
                <span className="text-muted-foreground">
                  {semanticAnalysisData.section_analyses.length} section{semanticAnalysisData.section_analyses.length !== 1 ? "s" : ""} analyzed
                </span>
            ...
            <span className="text-muted-foreground">{lines.length} lines • Click sections for details</span>
          </div>
        </div>
      )}
    </Card>
